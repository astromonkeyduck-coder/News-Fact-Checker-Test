(function(root,factory){
  ...
  return `<aside class="story-assistant" data-story-assistant data-context="${esc(JSON.stringify(context))}" hidden>
   <section class="sa-panel" id="story-ai-panel" aria-label="Ask Noteworthy AI about this story" hidden>
    ...
    <div class="sa-scroll"><div class="sa-context"><span>You’re reading</span><p>${esc(article.title)}</p></div>
     <section class="sa-call-panel" aria-label="Voice conversation" hidden><div class="sa-call-orb" aria-hidden="true">${mark}</div><h3>Talk through the story.</h3><p class="sa-call-status" role="status">Connecting your voice call…</p><p class="sa-call-disclosure">Your microphone audio and this story’s context are shared with OpenAI during the call. The AI can make mistakes.</p><div class="sa-call-controls"><button type="button" data-call-mute aria-pressed="false">Mute microphone</button><button type="button" data-call-end>End call</button><button type="button" data-call-retry hidden>Try again</button></div></section>
 ...
 function mount(doc,win){
  ...
  const speech=win.NoteworthyStorySpeech?.createSpeech({synthesis:win.speechSynthesis,Utterance:win.SpeechSynthesisUtterance,onChange:({state,message})=>{if(speechStop)speechStop.hidden=!['starting','speaking'].includes(state);if(speechReplay)speechReplay.hidden=!lastAnswer||['starting','speaking'].includes(state);if(speechStatus)speechStatus.textContent=message||(state==='speaking'?'Reading aloud.':state==='starting'?'Preparing spoken answer…':'');}});
  ...
  function readAnswer(){if(!lastAnswer||!speech?.supported){if(speechStatus)speechStatus.textContent='Spoken answers are unavailable in this browser.';return;}claimAudio('story-assistant');speech.speak(lastAnswer);}
  ...
  async function send(question){
   ...
   input.value=value;if(!input.reportValidity())return;pending=true;root.classList.add('has-conversation');root.querySelectorAll('[data-story-question]').forEach(b=>b.disabled=true);form.querySelector('button').disabled=true;input.readOnly=true;
   ...
   finally{pending=false;root.classList.remove('is-thinking');form.querySelector('button').disabled=false;input.readOnly=false;root.querySelectorAll('[data-story-question]').forEach(b=>b.disabled=false);}
  }
