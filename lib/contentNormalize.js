...
(function (root, factory) {
   ...
   * @param {Object} post
   * @param {Object} [opts]
   * @param {number} [opts.maxLength] Optional card-only limit; full headlines are the default.
   * @returns {string}
   */
  function cleanHeadline(post, opts) {
    if (!post || typeof post !== 'object') return 'Untitled';
    const maxLength = opts && Number.isFinite(opts.maxLength) && opts.maxLength > 0 ? opts.maxLength : null;

    const rawTitle = firstNonEmpty(post.title, post.headline);
    let candidate = stripUrls(rawTitle);
    // Legacy FDA titles shortened the product name inside the headline. The
    // compact source field can still hold that complete name. Expand only an
    // exact matching product prefix, leaving every surrounding word intact.
    const fullProduct = post.food_safety_summary && post.food_safety_summary.product;
    if (typeof fullProduct === 'string' && !/(?:…|\.\.\.)/.test(fullProduct) && candidate.includes('…')) {
      const split = candidate.indexOf('…');
      const before = candidate.slice(0, split);
      for (let length = Math.min(before.length, fullProduct.length - 1); length >= 20; length--) {
        if (before.slice(-length).toLowerCase() === fullProduct.slice(0, length).toLowerCase()) {
          candidate = before.slice(0, -length) + fullProduct + candidate.slice(split + 1);
          break;
        }
      }
    }
    ...
    // Legacy X imports stored a truncated title but retained the full original
    // body. Recover only when the body matches the complete stored prefix.
    // Never infer a missing ending or replace an intentional edited headline.
    if (/(?:…|\.\.\.)$/.test(candidate)) {
      const prefix = candidate.replace(/(?:…|\.\.\.)$/, '').trim();
      const originalRaw = firstNonEmpty(post.story, post.text, post.content, post.Content);
      const original = normalizeSocialPostText(originalRaw);
      const firstLine = stripDuplicateSourceLabel((original.split('\n').find((line) => line.trim()) || '').trim());
      if (prefix.length >= 20 && firstLine.startsWith(prefix) && firstLine.length > prefix.length
          && !/(?:…|\.\.\.)$/.test(firstLine)) candidate = firstLine;
      // Some stored titles ended partway through a trailing URL. Recover only
      // when removing that fragment exactly equals the cleaned original line.
      const withoutUrlFragment = prefix.replace(/\s+(?:ht|htt|http|https)$/i, '');
      if (withoutUrlFragment !== prefix && withoutUrlFragment === firstLine && /https?:\/\//i.test(originalRaw)) candidate = firstLine;
    }
    ...
    return maxLength ? truncate(candidate, maxLength) : candidate;
  }
  ...
  function normalizeMedia(post) {
    ...
    // Video candidates - explicit fields plus videos misfiled in images[].
    const imagePool = []
