...
- NEVER make up or fabricate events that are not in the list above`;
    ...
    try {
      ...
      // Get writing style from Blobs (avoids 4KB env var limit)
        ...
        for (const toolCall of message.tool_calls) {
          ...
          if (toolName === 'search_web') {
            ...
            try {
              ...
              // Don't break the conversation - just note that search failed and continue
              toolResponse = `I attempted to search for "${toolArgs.query}" but the search service is currently unavailable. I'll provide information based on my knowledge instead.`;
              searchInProgress = false; // Search failed
              // Continue with the conversation even if search fails
            }
