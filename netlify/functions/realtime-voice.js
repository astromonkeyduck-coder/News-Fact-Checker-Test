...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const isElevenLabsVoice = voice && voice.startsWith('elevenlabs:');
      ...
      // (audio will be generated by ElevenLabs on the frontend)
      const openAIVoiceForSession = isElevenLabsVoice ? 'alloy' : voice;
      
      console.log(`[Realtime Voice] Voice requested: ${voice}, isElevenLabs: ${isElevenLabsVoice}, OpenAI voice for session: ${openAIVoiceForSession}`);
...
When a voice conversation starts, greet the user by saying "Hey, It's Noteworthy AI" in a friendly, welcoming tone.`;
        ...
        try {
              ...
              session: {
                ...
                audio: {
                  ...
                  output: {
                    ...
                    voice: openAIVoiceForSession, // Use OpenAI voice for session (ElevenLabs will handle audio separately)
                    speed: 1.0
                  }
                }
              }
          ...
          if (fetchError.name === 'AbortError') {
            console.error('❌ [GET] OpenAI API request timed out after 12 seconds');
            return {
              statusCode: 504,
              headers,
              body: JSON.stringify({ 
...
When a voice conversation starts, greet the user by saying "Hey, It's Noteworthy AI" in a friendly, welcoming tone.`;
        ...
        try {
          ...
          clearTimeout(timeoutId);
          if (fetchError.name === 'AbortError') {
            console.error('❌ [POST] OpenAI API request timed out after 12 seconds');
            return {
              statusCode: 504,
              headers,
              body: JSON.stringify({ 
