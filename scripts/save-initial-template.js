// Script to save the initial 11/26/25 newsletter template
// Run this once to populate the template system

const https = require('https');
const http = require('http');

const NEWSLETTER_KEY = process.env.NEWSLETTER_KEY || '';
const NETLIFY_FUNCTION_URL = process.env.NETLIFY_FUNCTION_URL || 'http://localhost:8888/.netlify/functions/newsletter-templates';

// The 11/26/25 newsletter template HTML (exact copy from getNewsletterHTMLWithPosts)
// This is the professional briefing format used for the Washington DC shooting coverage
...
async function saveTemplate() {
  try {
    ...
    const url = `${NETLIFY_FUNCTION_URL}?token=${encodeURIComponent(NEWSLETTER_KEY)}`;
    const response = await makeRequest(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-admin-token': NEWSLETTER_KEY,
      },
    }, templateData);
    ...
    if (response.status === 200 && response.data.success) {
      ...
      console.error('❌ Failed to save template:', response.data);
    }
  } catch (error) {
...
module.exports = { saveTemplate, templateData };
